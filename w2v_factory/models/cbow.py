...
class CBOW(Word2VecBase):
    ...
    def forward_ns(self, ctxs: torch.Tensor, ctx_lens: torch.Tensor,
                   targets: torch.Tensor, neg_targets: torch.Tensor) -> W2VOutput:
        """
        ctxs: [B, Lmax]
        ctx_lens: [B]
        targets: [B]
    ...
    def forward_hs(self, ctxs: torch.Tensor, ctx_lens: torch.Tensor,
        ...
        mask = torch.arange(B, device=ctxs.device).unsqueeze(1)
